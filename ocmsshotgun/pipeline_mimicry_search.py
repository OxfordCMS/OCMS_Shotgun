...
###############################################################################]
@follows(blast_search, mkdir("04_find_mimicry_contigs.dir"))
@transform(
    "01_prokka_output.dir/*/*.fna",
    regex("01_prokka_output.dir/(.+)/.+.fna"),
    r"04_find_mimicry_contigs.dir/\1.fasta",
)

def extract_contigs(infile, outfile):
    """Find the contig nucelotide sequence containing the of proteins identified
    ...
    tbl_file = re.sub("fna", "tbl", infile)
    ...
    # define sampleid
    sample_id = re.search(r"01_prokka_output.dir/(.+)/.+.fna", infile).group(1)
    ...
    # }
    fna_dict = {}
    for id_seq in fna_file.split(">")[1:]:
        id, *seq = id_seq.split("\n")
    ...
    # }
    tbl_dict = {}
    for contig in tbl_file.split(">Feature ")[1:]:
