...
taxa_abundance <- list()
iter_file <- list.files("../humann3.dir/", pattern = "merged_metaphlan_.*\\.tsv")
for(i in 1:length(iter_file)) {
  curr_level <- gsub("merged_metaphlan_", "", iter_file[i])
  curr_level <- gsub(".tsv", "", curr_level)
  curr_file <- file.path("../humann3.dir", iter_file[i])
  taxa_abundance[[curr_level]] <- read.csv(curr_file, sep="\t")
}
...
```{r pathcov-community-plot, fig.height=18/2.54, fig.width=15/2.54}
p_pathcov_community
```
...
```{r pathabund-community-plot, fig.height=18/2.54, fig.width=15/2.54}
