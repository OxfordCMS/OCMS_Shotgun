...
def mergeMetaphlanCounts(infiles, outfile):
    ...
    '''
    infiles = sorted(glob.glob("metaphlan.dir/*.metaphlan.profile.txt"))
    headers = [P.snip(os.path.basename(x), ".metaphlan.profile.txt") for x in infiles]
    headers_str = ",".join(headers)

    statement = f'''
        ocms_shotgun combine_tables
            --glob=metaphlan.dir/*.metaphlan.profile.txt
            --skip-titles
            --header-names="{headers_str}"
            -m 0
            -k 5
            -c 1
            --log=merged_metaphlan_counts.dir/merged_metaphlan_counts.log
            | gzip > {outfile}
    '''
